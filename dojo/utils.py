...
def do_dedupe_finding(new_finding, scan_type=None, *args, **kwargs):
    try:
        ...
        enabled = False
    if enabled:
        deduplicationLogger.info('dedupe for: ' + str(new_finding.id) +
        ...
        if hasattr(settings, 'DEDUPLICATION_ALGORITHM_PER_PARSER'):
            ...
            deduplicationLogger.info('scan_type for this finding is :' + scan_type)
            # Default algorithm
            ...
            if(deduplicationAlgorithm == settings.DEDUPE_ALGO_UNIQUE_ID_FROM_TOOL):
                deduplicationLogger.debug("Unique ID flow")
...
def deduplicate_unique_id_from_tool(new_finding):
    ...
    deduplicationLogger.info("Found " +
        str(len(existing_findings)) + " findings with same unique_id_from_tool" + "for the finding " + str(new_finding.title))
    for find in existing_findings:
        ...
        try:
            ...
            deduplicationLogger.info(str(e))
            continue
        break
...
def set_duplicate(new_finding, existing_finding):
    ...
    if existing_finding.id == new_finding.id:
        deduplicationLogger.info('Can not add duplicate to itself')
        raise Exception("Can not add duplicate to itself")
    deduplicationLogger.info('Setting new finding ' + str(new_finding.id) + ' as a duplicate of existing finding ' + str(existing_finding.id))
    ...
    deduplicationLogger.info('saving new finding: %d', new_finding.id)
    deduplicationLogger.info('finding title: '+ new_finding.title)
    super(Finding, new_finding).save()
    deduplicationLogger.info('saving existing finding: %d', existing_finding.id)
