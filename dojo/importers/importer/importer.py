...
class DojoDefaultImporter(object):
    ...
    def process_parsed_findings(self, test, parsed_findings, scan_type, user, active, verified, minimum_severity=None,
        ...
        i = 0
        for item in items:
            logger.debug("Items being imported "+ str(item))
            sev = item.severity
            ...
            item.created = now
            item.updated = now
            logger.info("Save finding without deduplication")
            item.save(scan_type= scan_type, dedupe_option=False)
            ...
            if settings.FEATURE_FINDING_GROUPS and item.finding_group:
                logger.info("Saving the item/finding with deduplication")
                item.save(scan_type= scan_type)
            else:
                logger.info("Saving the item/finding with deduplication")
                item.save(scan_type=scan_type,push_to_jira=push_to_jira)
        ...
        logger.debug("Length of the total findings" + str(len(new_findings)))
        return new_findings

    def close_old_findings(self, test, scan_date_time, user, push_to_jira=None,scan_type='Cloudsploit Scan'):
        old_findings = []
        ...
        if(deduplicationAlgorithm == settings.DEDUPE_ALGO_UNIQUE_ID_FROM_TOOL):
                new_hash_codes = test.finding_set.filter(is_mitigated=False).values('unique_id_from_tool')
                current_hash_codes=[item['unique_id_from_tool'] for item in new_hash_codes]
                if test.engagement.deduplication_on_engagement:
                    logger.info("Unique ID From tool : With Dedup on engagement" + str(test.engagement.deduplication_on_engagement))
                    Finding.objects
                    old_findings = Finding.objects.exclude(test=test) \
                                                    .exclude(unique_id_from_tool__in=current_hash_codes) \
                    ...
                    logger.info("Unique ID From tool : Without Dedup on engagement" + str(test.engagement.deduplication_on_engagement))
                    old_findings = Finding.objects.exclude(test=test) \
                                                    .exclude(unique_id_from_tool__in=current_hash_codes) \
                ...
                new_hash_codes = test.finding_set.filter(is_mitigated=False).values('hash_code')
                current_hash_codes=[item['hash_code'] for item in new_hash_codes]
                ...
                if test.engagement.deduplication_on_engagement:
                    logger.info("Hash code From tool : With Dedup on engagement" + str(test.engagement.deduplication_on_engagement))
                    old_findings = Finding.objects.exclude(test=test) \
                                                    .exclude(hash_code__in=current_hash_codes) \
                                                    .filter(test__engagement=test.engagement,
                                                        test__test_type=test.test_type,
                                                        active=True)
                else:
                    # TODO BUG? this will violate the deduplication_on_engagement setting for other engagements
                    logger.info("Hash code From tool : Without Dedup on engagement" + str(test.engagement.deduplication_on_engagement))
                    old_findings = Finding.objects.exclude(test=test) \
                                                    .exclude(hash_code__in=current_hash_codes) \
                                                    .filter(test__engagement__product=test.engagement.product,
                                                        test__test_type=test.test_type,
                                                        active=True)
        logger.debug("####Count of current findings are"+str(len(current_hash_codes)))
        logger.debug(str(current_hash_codes))
        mitigated_findings=test.finding_set.filter(is_mitigated=True)

        for old_finding in old_findings:
            logger.debug("old finding-->"+old_finding.title)
            try:
                match=mitigated_findings.get(hash_code=old_finding.hash_code)
                logger.debug("match found === %s and id is %s",str(match),str(match.title))
                old_finding.dismisser=match.dismisser
                old_finding.dismissed_date=match.dismissed_date
                old_finding.tags.add('dismissed')
            except ObjectDoesNotExist:
                logger.info("match not found maybe the issue is closed....")
            except MultipleObjectsReturned:
                logger.error("multiple hash codes detected")            
    ...
    def import_scan(self, scan, scan_type, engagement, lead, environment, active, verified, tags=None, minimum_severity=None,
        ...
        if test is None:
            # Try one more time to create the test
            test = self.create_test(scan_type, engagement, lead, environment, scan_date=scan_date, tags=tags,
                            version=version, branch_tag=branch_tag, build_id=build_id, commit_hash=commit_hash, now=now,
                            sonarqube_config=sonarqube_config)
        ...
        closed_findings = []
        if close_old_findings or scan_type == 'Dependabot Scan':
            logger.debug('IMPORT_SCAN: Closing findings no longer present in scan report')
            closed_findings = self.close_old_findings(test, scan_date_time, user=user, push_to_jira=push_to_jira, scan_type=scan_type)
