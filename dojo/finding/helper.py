...
@app.task
@dojo_model_from_id
def post_process_finding_save(finding, scan_type=None, dedupe_option=True, false_history=False, rules_option=True, product_grading_option=True,
    ...
    if dedupe_option:
        hashcode_exists = hasattr(finding, "hash_code")
        if hashcode_exists:
            if finding.hash_code is not None:
                if system_settings.enable_deduplication:
                    from dojo.utils import do_dedupe_finding
                    do_dedupe_finding(finding, scan_type, *args, **kwargs)
                else:
                    deduplicationLogger.info("skipping dedupe because it's disabled in system settings")
            else:
                deduplicationLogger.info("skipping dedupe because hash_code is None")
        else:
                deduplicationLogger.info("skipping dedupe because hash_code is not available for finding" + finding.title)
    if false_history:
