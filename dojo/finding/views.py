...
from dojo.utils import add_error_message_to_response, get_object_or_none, add_field_errors_to_response, add_success_message_to_response, close_external_issue, redirect, reopen_external_issue
import copy
...
from dojo.models import Finding, Finding_Group, Notes, NoteHistory, Note_Type, \
    ...
    FileAccessToken, GITHUB_PKey, GITHUB_Issue, Dojo_User, Cred_Mapping, Test, Product, User, Engagement,Test_Type
...
def product_types_open_findings(request, pid=None, scantype=None,eid=None, view=None):
    return product_types_findings(request, pid=pid, scantype=scantype, eid=eid, view=view, filter_name="Open", query_filter=finding_helper.OPEN_FINDINGS_QUERY, prefetch_type='open')

def product_open_findings(request, pid=None, scantype=None,eid=None, view=None):
    return product_findings(request, pid=pid, scantype=scantype, eid=eid, view=view, filter_name="Open", query_filter=finding_helper.OPEN_FINDINGS_QUERY, prefetch_type='open')
...
django_filter=open_findings_filter, prefetch_type='all'):
    ...
    if pid:
        ...
        print("DOJO/FINDINGS PID not exists")
...
def product_types_findings(request, pid=None, scantype=None, eid=None, view=None, filter_name=None, query_filter=None, order_by='numerical_severity',
django_filter=open_findings_filter, prefetch_type='all'):
    ...
    if pid:
        ...
        if scantype != 'ALL':
            scan_type_name = " ".join(scantype.split("_"))
            test_type = get_object_or_none(Test_Type, name=scan_type_name)
            print("Procesed scan type ", scan_type_name)
            print("Test Type ", test_type)
            if test_type:
                findings = findings.filter(test__engagement__product__prod_type=pid).filter(test__test_type=test_type.id)
            else:
                # Setting Product type as 0, so no findings shown in UI
                findings = findings.filter(test__engagement__product__prod_type=0)
                print("Findings", findings)
            print("Findings -->", findings)
            
        else:
            print('Scan Type ALL', scantype)
            print("Findings -->", findings)
            findings = findings.filter(test__engagement__product__prod_type=pid)
...
def product_findings(request, pid=None, scantype=None, eid=None, view=None, filter_name=None, query_filter=None, order_by='numerical_severity',
django_filter=open_findings_filter, prefetch_type='all'):
    ...
    if pid:
        ...
        if scantype != 'ALL':
            ...
            test_type = get_object_or_none(Test_Type, name=scan_type_name)
            if test_type:
                findings = findings.filter(test__engagement__product__id=pid).filter(test__test_type=test_type.id)
            else:
                # Setting Product type as 0, so no findings shown in UI
                findings = findings.filter(test__engagement__product__id=0)
                print("Findings", findings)
