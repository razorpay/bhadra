...
def simple_search(request):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            logger.info("Searched repo name " + str(original_clean_query) )
            prod = get_object_or_none(Product, name=original_clean_query)
            if prod:
                logger.info("Product found")
                return HttpResponseRedirect(reverse('view_product', args=(prod.id, )))
            else:
                logger.info("No product found")
                # return HttpResponse("Not able to find product")
