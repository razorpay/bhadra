...
class Finding(models.Model):
    ...
    #dependabot fields start
    priority = models.CharField(default="P2",
                                max_length=10,
                                verbose_name="Priority",
                                help_text="The Priority level of this flaw (P0, P1, P2).")
    fixed_version = models.CharField(null=True,
                                    blank=True,
                                    max_length=100,
                                    verbose_name="Version where issue is fixed",
                                    help_text="Version or range of versions where the reported vulnerability is fixed.")
    vulnerable_version = models.CharField(null=True,
                                        blank=True,
                                        max_length=100,
                                verbose_name="Versions which are affected by the issue",
                                help_text="Version or range of versions which has reported vulnerability.")
    dismissed_date = models.CharField(null=True,
                                    blank=True,
                                    max_length=50,
                                verbose_name="Date at which this issue is dismissed.",
                                help_text="Date at which this issue was dismissed.")
    dismisser = models.CharField(null=True,
                                blank=True,
                                max_length=100,
                                verbose_name="User who dismissed this issue",
                                help_text="User who dismissed this issue.")
    repository = models.CharField(null=True,
                                blank=True,
                                max_length=100,
                                verbose_name="Repository against which this issue is raised.",
                                help_text="The repository against which this issue is raised.")
    bu = models.CharField(null=True,
                        blank=True,
                        max_length=50,
                        verbose_name="BU associated with this repo.",
                        help_text="BU associated with this repo.")
    pod = models.CharField(null=True,
                            blank=True,
                            max_length=50,
                            verbose_name="pod associated with this repo.",
                            help_text="pod associated with this repo.")
    package_ecosystem = models.CharField(null=True,
                                        blank=True,
                                        max_length=50,
                                        verbose_name="the type of package manager used.",
                                        help_text="package manager used for these issues NPM COMPOSER etc.")
    #dependabot fields end ..
    ...
    def compute_hash_code(self, scan_type=None):
        if hasattr(settings, 'HASHCODE_FIELDS_PER_SCANNER') and hasattr(settings, 'HASHCODE_ALLOWS_NULL_CWE') and hasattr(settings, 'HASHCODE_ALLOWED_FIELDS'):
            ...
            deduplicationLogger.info("Scan type identified for calculating hash code " + str(scan_type))
            if (scan_type in settings.HASHCODE_FIELDS_PER_SCANNER):
                ...
                if (all(elem in settings.HASHCODE_ALLOWED_FIELDS for elem in hashcodeFieldsCandidate)):
                    ...
                    if (scan_type in settings.HASHCODE_ALLOWS_NULL_CWE):
                        if (settings.HASHCODE_ALLOWS_NULL_CWE[scan_type] or self.cwe != 0):
                            ...
                            deduplicationLogger.info("compute_hash_code:" + "cwe is 0, can't compute hash code")
                        ...
                        hashcodeFields = hashcodeFieldsCandidate
                        deduplicationLogger.info("compute_hash_code:" + " Else block Checked inside selected fields are allowed" + str(hashcodeFields))
                        if(self.cwe == 0):
                            deduplicationLogger.info("compute_hash_code:" + "accepting cwe is null") 
                ...
                return self.compute_hash_code_legacy()
            deduplicationLogger.info("computing hash_code for finding id " + str(self.id) + " for scan_type " + scan_type + " based on: " + ', '.join(hashcodeFields))
            fields_to_hash = ''
    ...
    def save(self, dedupe_option=True, scan_type=None, false_history=False, rules_option=True, product_grading_option=True,
        ...
        if dedupe_option:
            if (self.hash_code is not None):
                ...
                self.hash_code = self.compute_hash_code(scan_type=scan_type)
        ...
        if dedupe_option or false_history or issue_updater_option or product_grading_option or push_to_jira:
            if self.hash_code is None:
                self.hash_code = self.compute_hash_code(scan_type=scan_type)
                deduplicationLogger.info("Hash_code computed for finding: %s", self.hash_code)
            if self.hash_code is not None:
                deduplicationLogger.info("Calling the post processing for deduplication " + self.title)
                finding_helper.post_process_finding_save(self, dedupe_option=dedupe_option, scan_type=scan_type, false_history=false_history, rules_option=rules_option, product_grading_option=product_grading_option,
                issue_updater_option=issue_updater_option, push_to_jira=push_to_jira, user=user, *args, **kwargs)
            else:
                deduplicationLogger.info("Post processing not performed for this " + self.title)
        else:
            logger.debug('no options selected that require finding post processing')
        
        logger.info("Going out of save function")
